            ...
            if(is_array($filters)){
                foreach($filters as $filter){
                    if(function_exists($filter)) {
                        if ($filter=='htmlspecialchars' && C('DB_CHARSET')=='gbk') {
                            $data   =   is_array($data) ? array_map_recursive($filter,$data) : call_user_func_array($filter, array($data,ENT_COMPAT,"gb2312")); // 参数过滤
                        } else {
    ...
    if ($filter=='htmlspecialchars' && C('DB_CHARSET')=='gbk') {
        foreach ($data as $key => $val) {
            $result[$key] = is_array($val)
                ...
                : call_user_func_array($filter, array($val,ENT_COMPAT,"gb2312"));
        }
    } else {
        foreach ($data as $key => $val) {
            $result[$key] = is_array($val)
